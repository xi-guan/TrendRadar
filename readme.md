...
| [🎯 核心功能](#-核心功能) | [🛠️ 技术栈](#️-技术栈) | [🚀 快速开始](#-快速开始) | [🐳 Docker部署](#-docker-部署) |
|:---:|:---:|:---:|:---:|
| [🤖 AI分析专区](#-ai-智能分析部署) | [📝 更新日志](#-更新日志) | [🔌 MCP客户端](#-mcp-客户端) | [❓ 答疑与常见问题](#问题答疑与1元点赞) |
| [🔧 自定义监控平台](#自定义监控平台) | [📝 frequency_words.txt 配置](#frequencywordstxt-配置教程) | [⭐ 项目相关](#项目相关) | |
...
| 技术分类 | 技术栈 | 说明 |
|---------|--------|------|
| **开发语言** | Python 3.10+ | 主要编程语言 |
| **AI 集成** | FastMCP 2.0 | MCP (Model Context Protocol) 服务器实现，支持 AI 智能分析 |
| **HTTP 请求** | requests ≥2.32.5 | 网络请求与数据爬取 |
| **配置管理** | PyYAML ≥6.0.3 | YAML 配置文件解析 |
| **时区处理** | pytz ≥2025.2 | 时区转换与时间处理 |
| **通信协议** | websockets ≥13.0 | WebSocket 协议支持 |

### **数据来源**

- **newsnow API** - 基于 [newsnow](https://github.com/ourongxing/newsnow) 项目的多平台新闻聚合 API
- 支持 11+ 主流平台（知乎、微博、抖音、bilibili、百度、华尔街见闻等）

### **通知推送渠道**

| 推送方式 | 支持服务 | 传输协议 |
|---------|---------|---------|
| **邮件推送** | Gmail、QQ、Outlook、163、126、新浪、搜狐 | SMTP (SSL/TLS) |
| **即时通讯** | 企业微信、Telegram、钉钉、飞书 | Webhook / Bot API |
| **开源服务** | ntfy | HTTP/WebSocket |

### **部署方案**

| 部署方式 | 说明 | 适用场景 |
|---------|------|---------|
| 🐳 **Docker** | 基于 `python:3.10-slim` 镜像，支持 amd64/arm64 | 服务器部署、稳定运行 |
| 🐙 **GitHub Actions** | 自动化 CI/CD 执行 | 免费云端定时任务 |
| 📄 **GitHub Pages** | 静态 HTML 展示 | 在线热点查看 |
| 💻 **本地运行** | Windows/Mac/Linux 直接运行 | 快速测试、个人使用 |

### **定时任务调度**

- **Supercronic** v0.2.34 - 轻量级 cron 任务调度器（Docker 环境）
- 支持多架构（amd64/arm64）

### **构建工具**

- **Hatchling** - 现代 Python 项目构建后端
- **pyproject.toml** - PEP 518 标准项目配置文件

### **配置文件**

- `config/config.yaml` - 主配置文件（爬虫、推送、通知等设置）
- `config/frequency_words.txt` - 关键词监控配置
- 支持环境变量覆盖配置

### **前端展示**

- 静态 HTML (index.html) - 新闻热点可视化展示页面
